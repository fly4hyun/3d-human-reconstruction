...
from a0_train_data_loader_10v import TestDataset
...
for data in pbar:
    ...
    optimizer_smpl = torch.optim.Adam(parameters_smpl.parameters(), lr = 0.01)#, weight_decay = 1e-5)
    ...
    for i_num in iterations:
        ...
        smpl_out_45 = torch.cat([(smpl_out[:, :, 0] - smpl_out[:, :, 2]).unsqueeze(-1)  / 2 ** 0.5, smpl_out[:, :, 1].unsqueeze(-1), (smpl_out[:, :, 0] + smpl_out[:, :, 2]).unsqueeze(-1)  / 2 ** 0.5], dim = -1)
        #smpl_out = smpl_out * torch.tensor([1.0, -1.0, -1.0]).to(device)
        ...
        index_2 = index_2.expand(smpl_out.size(0), smpl_out.size(1), index_1.size(2))
        ...
        #T_mask_3 = (T_mask_3 > 0.00001) * 1.
        
        
        
        render.load_meshes(smpl_out_45, torch.tensor(faces.astype(np.int64)))
        smpl_img_4, _ = render.get_rgb_image()
        T_mask_4, _ = render.get_silhouette_image()
        #T_mask_4 = (T_mask_4 > 0.00001) * 1.
        
        render.load_meshes(smpl_out_45.gather(2, index_1) * torch.tensor([-1.0, 1.0, 1.0]).to(device), torch.tensor(faces.astype(np.int64)))
        smpl_img_5, _ = render.get_rgb_image()
        T_mask_5, _ = render.get_silhouette_image()
        #T_mask_5 = (T_mask_5 > 0.00001) * 1.
        
        render.load_meshes(smpl_out_45 * torch.tensor([-1.0, 1.0, -1.0]).to(device), torch.tensor(faces.astype(np.int64)))
        smpl_img_6, _ = render.get_rgb_image()
        T_mask_6, _ = render.get_silhouette_image()
        #T_mask_6 = (T_mask_6 > 0.00001) * 1.
        
        render.load_meshes(smpl_out_45.gather(2, index_1) * torch.tensor([1.0, 1.0, -1.0]).to(device), torch.tensor(faces.astype(np.int64)))
        smpl_img_7, _ = render.get_rgb_image()
        T_mask_7, _ = render.get_silhouette_image()
        #T_mask_7 = (T_mask_7 > 0.00001) * 1.
        
        
        
        # render.load_meshes(smpl_out.gather(2, index_2) * torch.tensor([1.0, -1.0, 1.0]).to(device), torch.tensor(faces.astype(np.int64)))
        # smpl_img_8, _ = render.get_rgb_image()
        # T_mask_8, _ = render.get_silhouette_image()
        
        # render.load_meshes(smpl_out.gather(2, index_2) * torch.tensor([1.0, 1.0, -1.0]).to(device), torch.tensor(faces.astype(np.int64)))
        # smpl_img_9, _ = render.get_rgb_image()
        # T_mask_9, _ = render.get_silhouette_image()
        ...
        img_4 = ((smpl_img_4[0].permute(1, 2, 0) + 1.0) * 255.0 / 2.0).detach().cpu().numpy().astype(np.uint8)
        Image.fromarray(img_4).save('./_4.png')
        img_5 = ((smpl_img_5[0].permute(1, 2, 0) + 1.0) * 255.0 / 2.0).detach().cpu().numpy().astype(np.uint8)
        Image.fromarray(img_5).save('./_5.png')
        img_6 = ((smpl_img_6[0].permute(1, 2, 0) + 1.0) * 255.0 / 2.0).detach().cpu().numpy().astype(np.uint8)
        Image.fromarray(img_6).save('./_6.png')
        img_7 = ((smpl_img_7[0].permute(1, 2, 0) + 1.0) * 255.0 / 2.0).detach().cpu().numpy().astype(np.uint8)
        Image.fromarray(img_7).save('./_7.png')
        # img_8 = ((smpl_img_8[0].permute(1, 2, 0) + 1.0) * 255.0 / 2.0).detach().cpu().numpy().astype(np.uint8)
        # Image.fromarray(img_8).save('./_8.png')
        # img_9 = ((smpl_img_9[0].permute(1, 2, 0) + 1.0) * 255.0 / 2.0).detach().cpu().numpy().astype(np.uint8)
        # Image.fromarray(img_9).save('./_9.png')
        ...
        img_4_0 = ((data['image_4'][0].permute(1, 2, 0) + 1.0) * 255.0 / 2.0).detach().cpu().numpy().astype(np.uint8)
        Image.fromarray(img_4_0).save('./_4_0.png')
        img_5_0 = ((data['image_5'][0].permute(1, 2, 0) + 1.0) * 255.0 / 2.0).detach().cpu().numpy().astype(np.uint8)
        Image.fromarray(img_5_0).save('./_5_0.png')
        img_6_0 = ((data['image_6'][0].permute(1, 2, 0) + 1.0) * 255.0 / 2.0).detach().cpu().numpy().astype(np.uint8)
        Image.fromarray(img_6_0).save('./_6_0.png')
        img_7_0 = ((data['image_7'][0].permute(1, 2, 0) + 1.0) * 255.0 / 2.0).detach().cpu().numpy().astype(np.uint8)
        Image.fromarray(img_7_0).save('./_7_0.png')
        # img_8_0 = ((data['image_8'][0].permute(1, 2, 0) + 1.0) * 255.0 / 2.0).detach().cpu().numpy().astype(np.uint8)
        # Image.fromarray(img_8_0).save('./_8_0.png')
        # img_9_0 = ((data['image_9'][0].permute(1, 2, 0) + 1.0) * 255.0 / 2.0).detach().cpu().numpy().astype(np.uint8)
        # Image.fromarray(img_9_0).save('./_9_0.png')
        ...
        diff_loss_4 = torch.abs(smpl_img_4[0] - data['image_4'][0].to(device))
        diff_loss_5 = torch.abs(smpl_img_5[0] - data['image_5'][0].to(device))
        diff_loss_6 = torch.abs(smpl_img_6[0] - data['image_6'][0].to(device))
        diff_loss_7 = torch.abs(smpl_img_7[0] - data['image_7'][0].to(device))
        # diff_loss_8 = torch.abs(smpl_img_8[0] - data['image_8'][0].to(device))
        # diff_loss_9 = torch.abs(smpl_img_9[0] - data['image_9'][0].to(device))

        
        normal_loss = ((diff_loss_0 + diff_loss_1 + diff_loss_2 + diff_loss_3 + diff_loss_4 + diff_loss_5 + diff_loss_6 + diff_loss_7) / 2).mean() + diff_loss_0.mean()
        # normal_loss = (diff_loss + diff_loss).mean()
        
        # silhouette loss
        smpl_arr = torch.cat([T_mask_0, T_mask_1, T_mask_2, T_mask_3, T_mask_4, T_mask_5, T_mask_6, T_mask_7], dim=-1)[0]
        gt_arr = torch.cat([data['image_0'][0], data['image_1'][0], data['image_2'][0], data['image_3'][0], data['image_4'][0], data['image_5'][0], data['image_6'][0], data['image_7'][0]], dim=2).permute(1, 2, 0)
        ...
        smpl_arr_0 = torch.cat([T_mask_0, T_mask_0], dim=-1)[0]
        gt_arr_0 = torch.cat([data['image_0'][0], data['image_0'][0]], dim=2).permute(1, 2, 0)
        gt_arr_0 = ((gt_arr_0 + 1.0) * 0.5).to(device)
        bg_color_0 = (torch.Tensor([0.5, 0.5, 0.5]).unsqueeze(0).unsqueeze(0).to(device))
        gt_arr_0 = ((gt_arr_0 - bg_color_0).sum(dim=-1) != 0.0).float()
        diff_S_0 = torch.abs(smpl_arr_0 - gt_arr_0)


        
        silhouette_loss = diff_S.mean() + diff_S_0.mean()
        ...
        if i_num < 100:
            loss = 0.5 * normal_loss + 0.5 * silhouette_loss + 0.001 * loss_smpl(smpl_out, data['verts'].to(device))# + 1 * arm_loss#loss_smpl(smpl_out.to(device), data['verts'].to(device))
        else:
            loss = 0.5 * normal_loss + 0.5 * silhouette_loss + 0.01 * loss_smpl(smpl_out, data['verts'].to(device))# + 1 * arm_loss#loss_smpl(smpl_out.to(device), data['verts'].to(device))
