...
#from renderlib.render_mesh_utils import get_visibility
from models_SMPL_ICON.mesh_util import get_visibility

import renderlib.render_utils as util
import torch
import numpy as np
...
def query_color_4v(verts, faces, image_0, image_2, device):
    """query colors from points and image
    ...
    uv = uv * torch.tensor([1.0, -1.0]).type_as(uv)
    colors = (
        torch.nn.functional.grid_sample(image_0, uv, align_corners=True)[0, :, :, 0].permute(1, 0) +
        1.0
    ) * 0.5 * 255.0
    ...
    xy_2 = xy * torch.tensor([-1.0, -1.0]).to(device)
    visibility_2 = get_visibility(xy_2, z, faces[:, [0, 2, 1]]).flatten()
    uv_2 = xy_2.unsqueeze(0).unsqueeze(2)    # [B, N, 2]
    uv_2 = uv_2 * torch.tensor([1.0, -1.0]).type_as(uv_2)
    
    colors_2 = (
        torch.nn.functional.grid_sample(image_2, uv_2, align_corners=True)[0, :, :, 0].permute(1, 0) +
        1.0
    ) * 0.5 * 255.0
    
    colors_2[visibility_2 == 0.0] = (
        (Meshes(verts.unsqueeze(0), faces.unsqueeze(0)).verts_normals_padded().squeeze(0) + 1.0) *
        0.5 * 255.0
    )[visibility_2 == 0.0]
    
    colors[visibility == 0.0] = colors_2[visibility == 0.0]
