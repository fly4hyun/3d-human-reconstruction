...
from .utils import (
    Struct, to_np, to_tensor, Tensor, Array, SMPLOutput, SMPLHOutput, SMPLXOutput, MANOOutput,
    FLAMEOutput, find_joint_kin_chain
)
from .vertex_joint_selector import VertexJointSelector

ModelOutput = namedtuple(
    'ModelOutput', [
        'vertices', 'joints', 'full_pose', 'betas', 'global_orient', 'body_pose', 'expression',
...
class SMPL(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # shapedirs = data_struct.shapedirs # (6890, 3, 10)
        shapedirs = np.load('./model_save/SMPL/shapedirs_300.npy') # (6890, 3, 300)
        # shapedirs = np.reshape(shapedirs, [-1, 300]).T # (300, 20670)
...
class SMPLX(SMPLH):
    ...
    def create_mean_pose(self, data_struct, flat_hand_mean=False):
        ...
        pose_mean = np.concatenate(
            [
                global_orient_mean, body_pose_mean, jaw_pose_mean, leye_pose_mean, reye_pose_mean,
    ...
    def forward(
        ...
        **kwargs
        ...
        full_pose = torch.cat(
            [
                global_orient, body_pose, jaw_pose, leye_pose, reye_pose, left_hand_pose,
                right_hand_pose
            ],
            dim=1
        )
...
class SMPLXLayer(SMPLX):
    ...
    def forward(
        ...
        **kwargs
        ...
        model_vars = [
            betas, global_orient, body_pose, transl, expression, left_hand_pose, right_hand_pose,
            jaw_pose
        ]
        batch_size = 1
        for var in model_vars:
