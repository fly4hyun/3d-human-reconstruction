...
def getBicepPoint(sections):
    location_percentage = 86.25 # percentage
    approximate_location = math.floor(location_percentage*len(sections)/100)
    section_min = approximate_location - 100
    section_max = approximate_location + 0
    range_sections = range(section_max, section_min, -1)
...
def getBicep(sections, bicep_location):
    thigh_location = bicep_location - 1
    polygon = getSmallerAreaPolygon(sections[thigh_location])
    return sections[thigh_location], thigh_location, polygon.length

def getCrotch(sections):
    location_percentage = 47 # percentage
    approximate_location = math.floor(location_percentage*len(sections)/100)
    section_min = approximate_location - 25
    section_max = approximate_location + 25
    range_sections = range(section_min, section_max)
    ...
    for index in range_sections:
        if stop == False:
            now_polygon = getLargerAreaPolygon(sections[index])
            now_Y = getMaxY(now_polygon) - getMinY(now_polygon)
            before_polygon = getLargerAreaPolygon(sections[index - 1])
            before_Y = getMaxY(before_polygon) - getMinY(before_polygon)
            # if len(sections[index].entities) == 1:
            if now_Y > before_Y * 1.2:
                crotch = sections[index]
                position = index
                length = sections[index].polygons_closed[0].length
                stop = True
...
def getSmallerAreaPolygon(section):
    higher = 999
    polygon = None
    for pol in section.polygons_closed:
        if pol != None:
...
class Body3D(object):
    def __init__(self, vertices, faces, steps=0.005, levels=[-1.5, 1.5]):
        ...
        _, self.bicep_point_location, _ = getBicepPoint(self.sections)
        _, self.hip_location, _ = getHip(self.sections, self.crotch_location)
        _, self.bicep_location, _ = getBicep(self.sections, self.bicep_point_location)
    ...
    def getMeasurements(self):
        ...
        _, bicep_point_location, _ = getBicepPoint(self.sections)
        _, bicep_location, bicep_length = getBicep(self.sections, self.bicep_point_location)
        return weight, height, shoulder_length, chest_length, hip_length, waist_length, thigh_length, outer_leg_length, inner_leg_length, neck_length, neck_hip_length, bicep_length
    ...
    def bicep(self):
        return getBicep(self.sections, self.bicep_point_location)