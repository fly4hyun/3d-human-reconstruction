...
from data_load_for_app import TestDataset
...
def test(name):
    ...
    LVD_encoder_save_dir = '/jf-training-home/src/model_save/LVD_encoder'
    ...
    SMPL_model = SMPL('/jf-training-home/src/model_save/LVD_SMPL/neutral_smpl_with_cocoplus_reg.txt', obj_saveable = True).to(device)
    #SMPL = SMPL('/jf-training-home/src/model_save/LVD_SMPL/neutral_smpl_with_cocoplus_reg.txt').to(device)
    ...
    cfg.merge_from_file('/jf-training-home/src/model_save/ICON_img/icon-filter.yaml')
    cfg_show_list = [
    ...
    cfg.merge_from_file('/jf-training-home/src/model_save/ICON_img/pymaf_config.yaml')
    ...
    for data in pbar:
        ...
        in_tensor = {'image': data['image'], 'image_tensor': data['image_tensor'], 'smpl_faces': torch.as_tensor([SMPL_model.faces]).long(), 'name': data['name']}
        ...
        with torch.no_grad():
            ...
            m.export('/jf-training-home/src/app_result/0_LVD_%s.obj'%data['name'])
        ...
        if mode_smpl == 'icon':
            ...
            prior = MaxMixturePrior(prior_folder='/jf-training-home/src/model_save/LVD_SMPL/', num_gaussians=8) #.get_gmm_prior()
            prior = prior.to(device)
        ...
        for i in iterations:
            ...
            if mode_smpl == 'icon':
                ...
                vertices_smpl = (SMPL_model(body_pose=optimed_pose, betas=optimed_betas, global_orient = optimed_orient, pose2rot = False).vertices + optimed_trans)*optimed_scale
            elif mode_smpl == 'lvd':
                optimed_pose, optimed_betas, optimed_trans, optimed_scale = parameters_smpl.forward()
                vertices_smpl = (SMPL_model.forward(theta=optimed_pose, beta=optimed_betas, get_skin=True)[0][0] + optimed_trans)*optimed_scale
            ...
            loss = loss + prior_loss * 0.000001 + beta_loss * factor_beta_reg + normal_loss + silhouette_loss
        ...
        m = trimesh.Trimesh(pred_mesh, in_tensor['smpl_faces'].detach().cpu()[0], process = False, maintains_order = True)
        m.export('/jf-training-home/src/app_result/1_LVD_%s.obj'%data['name'])
        ...
        recon_obj.export(os.path.join('/jf-training-home/src/app_result/2_ICON_%s_recon.obj'%data['name']))


        verts_refine, faces_refine = remesh(os.path.join('/jf-training-home/src/app_result/2_ICON_%s_recon.obj'%data['name']), 0.5, device)
        ...
        final.export(os.path.join('/jf-training-home/src/app_result/4_ICON_%s_refine.obj'%data['name']))
