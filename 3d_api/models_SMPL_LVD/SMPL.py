...
class SMPL(nn.Module):
    def __init__(self, model_path, joint_type = 'cocoplus', obj_saveable = False):
        ...
        np_shapedirs = np.reshape(np_shapedirs, [-1, 10]).T
        self.register_buffer('shapedirs', torch.from_numpy(np_shapedirs).float())

        np_shapedirs_300 = np.load('./model_save/LVD_SMPL/shapedirs_300.npy')
        np_shapedirs_300 = np.reshape(np_shapedirs_300, [-1, 300]).T
        self.register_buffer('shapedirs_300', torch.from_numpy(np_shapedirs_300).float())
    ...
    def forward(self, beta, theta, get_skin = False, theta_in_rodrigues=True):
        ...
        if size_beta == 10:
            ...
            v_shaped = torch.matmul(beta, self.shapedirs_300).view(-1, self.size[0], self.size[1]) + self.v_template
    ...
    def unpose_and_deform_cloth_w_normals(self, v_cloth_posed, v_normals, theta_from, theta_to, beta, Jsmpl, vsmpl, theta_in_rodrigues=True):
        device = theta_from.device
        self.cur_device = torch.device(device.type, device.index)
        num_batch = beta.shape[0]
...
def getSMPL():
    return SMPL(os.path.normpath(os.path.join(os.path.dirname(__file__),'./model_save/LVD_SMPL/neutral_smpl_with_cocoplus_reg.txt')), obj_saveable = True)
def getTmpFile():
...
if __name__ == '__main__':
    ...
    smpl = SMPL(os.path.join(os.path.dirname(__file__),'./model_save/LVD_SMPL/neutral_smpl_with_cocoplus_reg.txt'), obj_saveable = True).to(device)
    pose= np.array([
